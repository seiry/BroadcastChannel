...
export async function getChannelInfo(context: RequestContext, params: GetChannelInfoParams = {}): Promise<ChannelInfo> {
  ...
  const overrideSiteAvatarUrl = getEnv(import.meta.env, context, 'OVERRIDE_SITE_AVATAR_URL')
  const overrideSiteTitle = getEnv(import.meta.env, context, 'OVERRIDE_SITE_TITLE')
  const overrideSiteDescription = getEnv(import.meta.env, context, 'OVERRIDE_SITE_DESCRIPTION')
  const overrideSiteDescriptionHtml = getEnv(import.meta.env, context, 'OVERRIDE_SITE_DESCRIPTION_HTML')

  const channelInfo: ChannelInfo = {
    posts,
    title: overrideSiteTitle || $('.tgme_channel_info_header_title').text(),
    description: overrideSiteDescription || $('.tgme_channel_info_description').text(),
    descriptionHTML: overrideSiteDescriptionHtml || (await modifyHTMLContent($, $('.tgme_channel_info_description'), { staticProxy })).html(),
    avatar: overrideSiteAvatarUrl || $('.tgme_page_photo_image img').attr('src'),
  }
